...
export class StepLoadModel extends EventTarget {
  ...
  private calculate_geometry_list(): void {
    ...
    this.final_mesh_data.traverse((child: Object3D) => {
      if (child.type === "Mesh") {
        const geometry_to_add: BufferGeometry = (
          child as Mesh
        ).geometry.clone();
    ...
    console.log(
      `Vertex count:${this.vertex_count}    Triangle Count:${this.triangle_count}     Object Count:${this.objects_count} `
    );
  }
  ...
  public add_event_listeners(): void {
    if (this.ui.dom_upload_model_button !== null) {
      this.ui.dom_upload_model_button.addEventListener(
        "change",
        (event: Event) => {
    ...
    if (this.ui.dom_load_model_debug_checkbox !== null) {
      this.ui.dom_load_model_debug_checkbox.addEventListener(
        "change",
        (event: Event) => {
    ...
    if (this.ui.dom_load_model_button !== null) {
      this.ui.dom_load_model_button.addEventListener("click", () => {
        ...
        if (model_selection !== null) {
          const file_name =
            model_selection.options[model_selection.selectedIndex].value;
  ...
  private get_file_extension(file_path: string): string {
    ...
    if (file_name === undefined) {
      console.error(
        "Critical Error: Undefined file extension when loading model"
      );
      return "UNDEFINED";
    }
    ...
    if (file_extension === undefined) {
      console.error(
        'Critical Error: File does not have a "." symbol in the name'
      );
      return "UNDEFINED";
    }
    ...
    if (file_extension === "fbx") {
      ...
      this.fbx_loader.load(
        ...
        (error) => {
          ...
          this.dispatchEvent(
            new CustomEvent("modelLoadError", {
              detail: { error: error.message || "Failed to load FBX" },
            })
          );
        }
      );
      ...
      this.gltf_loader.load(
        ...
        (error) => {
          ...
          this.dispatchEvent(
            new CustomEvent("modelLoadError", {
              detail: { error: error.message || "Failed to load GLB" },
            })
          );
        }
      );
      ...
      console.error(
        "Unsupported file format to load. Only acccepts FBX, GLTF, GLB:",
        model_file_path
      );
      this.dispatchEvent(
        new CustomEvent("modelLoadError", {
          detail: { error: "Unsupported file format" },
        })
      );
    }
  }
  ...
  private process_loaded_scene(loaded_scene: Scene): void {
    ...
    const clean_scene_with_only_models =
      this.strip_out_all_unecessary_model_data(this.original_model_data);
  ...
  private scale_model_on_import_if_extreme(scene_object: Scene): void {
    ...
    if (height > 0.1 && height < 4) {
      console.log(
        "Model a reasonable size, so no scaling applied: ",
        height,
        " height"
      );
      return;
      ...
      console.log(
        "Model is very large or small, so scaling applied: ",
        height,
        " height"
      );
    }
